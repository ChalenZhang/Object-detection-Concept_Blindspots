...
def score(model, inputs, device):
    ...
    for begin in range(0,len(inputs),256):
        ...
        count=torch.expm1(output["log_error_count"].clamp(-8,8)).clamp_min(0)
...
def main():
    ...
    config_name="training_fp.json" if args.task=="fp" else "training.json"
    conf=json.loads((args.configs/config_name).read_text())
    if args.task=="fp" and conf["q50_weight"]!=0:
        parser.error("FP training requires q50_weight=0")
    ...
    target_key="fp_targets" if args.task=="fp" else "targets"
    count_key="error_count" if args.task=="fp" else "miss_count"
    binary_key="has_error" if args.task=="fp" else "has_miss"
    targets=train[target_key]
    cal_miss=calibration[target_key][count_key][:,:2].sum(dim=1)
    ...
    positive=targets[binary_key].sum(dim=0)
    ...
    for seed in args.seeds:
        ...
        for multiplier in conf["lr_multipliers"]:
            ...
            for step in range(1,steps+1):
                ...
                binary=targets[binary_key][index].to(args.device)
                count=torch.log1p(targets[count_key][index].to(args.device))
                model.train();output=model(x)
                loss=F.binary_cross_entropy_with_logits(output["binary_logits"],binary,pos_weight=pos_weight)
                loss=loss+F.smooth_l1_loss(output["log_error_count"],count)
                if conf["q50_weight"]:
                ...
                rank=torch.stack([ranking_loss(output["log_error_count"][:,i],count[:,i]) for i in range(len(GROUPS))]).mean()
            ...
            payload["failure_target"]=args.task
            (args.output/f"training_{args.task}_seed_{seed}_lr_{multiplier:g}.json").write_text(json.dumps(history,indent=2)+"\n")
        chosen=max(candidates,key=lambda row:(row[0],row[1]))[2]
        prefix="risk_fp" if args.task=="fp" else "risk"
        torch.save(chosen,args.output/f"{prefix}_seed_{seed}.pt")
